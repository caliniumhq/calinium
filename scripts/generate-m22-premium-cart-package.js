...
/*
 * Read-only, fixture-backed package proof for Milestone 22.
 * It never calls Shopify and refuses to overwrite a prior generation workspace.
 */
const fs = require('fs');
...
const generationId = 'generation-run-m22-premium-cart-final-20260727-r4';

function fixtureGenerationContext(draft) {
  const merchantReferences = {};
  const assetReferences = Object.fromEntries((draft.required_assets.required || []).map((asset) => [asset.asset_id, `shopify://fixture-assets/m22-premium-cart-${asset.asset_id}`]));
    ...
    for (const section of page.sections || []) {
      for (const field of section.unresolved_merchant_fields || []) {
        ...
        if (requiresReference) {
          merchantReferences[field.setting_ref] = `shopify://fixture-resources/m22-premium-cart-${field.setting_ref.replace(/[^a-z0-9]+/gi, '-')}`;
  ...
  return {
    ...
    approval_reference: 'm22-premium-cart-fixture-approved-inputs',
...
function main() {
  ...
  if (result.status !== 'generated_for_review' || !result.read_only_theme_package?.validation?.valid) {
    throw new Error(`Expected a valid read-only package, received ${result.status}.`);
  }
