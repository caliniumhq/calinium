...
const generationId = 'generation-run-m21-premium-collection-final-20260726-r2';

function fixtureGenerationContext(draft) {
  const merchantReferences = {};
  const assetReferences = Object.fromEntries((draft.required_assets.required || []).map((asset) => [asset.asset_id, `shopify://fixture-assets/m21-premium-collection-${asset.asset_id}`]));
  ...
  for (const page of [draft.homepage_plan, ...(draft.other_pages || [])]) {
    for (const section of page.sections || []) {
      for (const field of section.unresolved_merchant_fields || []) {
        const requiresReference = requiredFieldRefs.has(field.setting_ref) || ['collection', 'product', 'blog', 'link_list', 'menu', 'page', 'article', 'image', 'desktop_image', 'mobile_image', 'poster_image', 'video', 'video_url'].includes(field.setting_id);
        if (requiresReference) {
          merchantReferences[field.setting_ref] = `shopify://fixture-resources/m21-premium-collection-${field.setting_ref.replace(/[^a-z0-9]+/gi, '-')}`;
  ...
  return {
    ...
    approval_reference: 'm21-premium-collection-fixture-approved-inputs',
    approved_at: '2026-07-26T23:30:00.000Z',
